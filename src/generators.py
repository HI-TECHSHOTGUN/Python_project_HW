...
def filter_by_currency(transactions, currency_code: str):
    """Функция для фильтрации списка по валюте"""
    for transaction in transactions:
        if transaction["operationAmount"]["currency"]["code"] == currency_code:
            yield transaction

def transaction_descriptions(transactions):
    """Функция для прописывания пути перевода"""
    for transaction in transactions:
...
def card_number_generator(start: int, stop: int):
    """Функция для генерации банковской карты в определенном диапазоне"""
    if not (0 <= start <= 9999999999999999 and 0 <= stop <= 9999999999999999):
